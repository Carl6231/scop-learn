...
retention <- do.call(rbind, lapply(names(objects), function(id) {
  ...
  data.frame(
    sample_id = id,
    input_cells = nrow(md),
    doublets = sum(md$db_qc %in% FALSE, na.rm = TRUE),
    qc_pass = sum(md$CellQC %in% TRUE, na.rm = TRUE)
  )
}))
```

Column names can differ by the selected `RunCellQC()` metrics and SCOP version; inspect the returned metadata rather than copying the last two lines blindly. A sample that loses 80% of cells is a study-level warning that must remain visible.
