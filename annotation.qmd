...
```r
# requires SingleR plus a labelled Seurat reference
srt <- RunSingleR(
  srt_query = srt,
...
A per-cluster consensus table should contain the cluster, positive markers, contradictory markers, reference prediction and score, CellTypist label and confidence, final label, uncertainty flag, and reviewer note. The published evidence records method-level outputs; a new study must fill this table with its own cluster evidence rather than copy the bundled truth labels.
