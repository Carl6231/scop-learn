...
在现有独立实验单位、重复、object fields 与已安装 backend 下，预先规定的哪个生物学问题真正可识别？只有同时具备 input contract、完整结果表、sensitivity check、SCOP plot 与 stopping rule，才能保留一个高级模块。
...
| 哪些群体响应更强，或 virtual knockout 预测什么？ | replicated perturbation labels 或有依据 network | response priority / in-silico hypothesis | `RunAugur()` 或 `RunscTenifoldKnk()` → `scTenifoldKnkPlot()` |
...
## 9 · Perturbation response 与 virtual knockout

`RunAugur()` 根据两个 perturbation/condition labels 的可分性排序 cell populations，但不能替代 sample-level effect estimation：
...
Network-based **in-silico** knockout 可使用：
...
Augur score 是 predictive separability，不是 effect size/causality；virtual knockout 是 network-model prediction，不是 CRISPR experiment。真实 perturb-seq 必须有 biological replication、guide/perturbation QC，并在 causal statement 前做实验验证。

## 10 · 在空间数据中验证单细胞结果

只有当 single-cell 与 spatial 材料在 species、tissue、condition、anatomical context、feature IDs 和 coordinate quality 上兼容时，空间验证才成立。教学切片中的匹配 gene pattern 只能作为 localization 的正交支持，不能充当 patient-level replication，也不能证明一个 spot 只有一种细胞。

::: {.input-contract}
**空间验证输入合同**

- 含 raw 或 normalized `Spatial` assay layers 的 spatial Seurat object；
- 已声明 coordinate columns 和 image/section identity；
- 预先规定的 single-cell signature 或 candidate gene，而不是对所有结果做事后搜索；
- species 匹配且 feature identifiers 可解析；
- tissue 与 condition 兼容，否则明确写为 exploratory comparison；
- 若需要 deconvolution，必须有带已审核 labels 的 cell reference。
:::

先用 SCOP 检查预先规定的 candidate 是否具有空间结构：
...
若科学问题是 cell-type composition 而不是 spot expression，deconvolution 属于另一证据层级：
...
真实 human PanIN object 上已执行 `RunSpatialVariableFeatures()` 与对应 SCOP plotter，详见[空间章节](spatial-case.qmd)。`RunRCTD()` 已核对 SCOP 0.8.9 signature，但审计环境缺少 `spacexr`，因此没有执行；不能用 mock plot 替代。若 tissue/condition 不兼容、coordinate QC 失败、candidate 不存在、pattern 被 depth/edge artefact 驱动，或 optional backend 不可用，必须停止验证。
...
| perturbation prediction → causal claim | replicated perturbation 与 experimental validation | 只有 classifier/network prediction |
